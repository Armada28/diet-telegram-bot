...
async def греч_мем(message: Message):
    ...
    # Проверяем, что это про еду, а не про Грецию
    if any(word in txt for word in ["съел", "сел", "скушал", "поел", "ем", "ем греч", "жру", "закинул", "грамм", "гр ", "кило", "порцию"]):
        # тут твои смешные ответы
        await message.reply("Гречка level 100 активирован 🥣💪")
    else:
        # опционально: иногда подкалывать за Грецию
